<?php
/**
 * Created by PhpStorm.
 * User: bwalleshauser
 * Date: 2/11/2015
 * Time: 2:15 PM
 */

class Mainstreethost_DynamicOriginShipping_Adminhtml_OriginController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Instantiate our grid container block and add to the page content.
     * When accessing this admin index page, we will see a grid of all
     * brands currently available in our Magento instance, along with
     * a button to add a new one if we wish.
     */
    public function indexAction()
    {
        // instantiate the grid container
        $originBlock = $this->getLayout()
            ->createBlock('dos/adminhtml_origin');

        $remaining = count(Mage::helper('dos')->GetRemainingRegions());
        if($remaining > 0)
        {
            Mage::getSingleton('core/session')->addWarning("There " . (($remaining > 1) ? "are " : "is ") . $remaining . " unmapped region" . (($remaining > 1) ? "s" : "") . ".");
        }


        // Add the grid container as the only item on this page
        $this->loadLayout()
            ->_addContent($originBlock)
            ->renderLayout();
    }

    /**
     * This action handles both viewing and editing existing brands.
     */
    public function editAction()
    {
        /**
         * Retrieve existing brand data if an ID was specified.
         * If not, we will have an empty brand entity ready to be populated.
         */
        $origin = Mage::getModel('dos/origin');
        if ($originId = $this->getRequest()->getParam('id', false))
        {
            $origin->load($originId);

            if (!$origin->getId())
            {
                $this->_getSession()->addError(
                    $this->__('This origin no longer exists.')
                );
                return $this->_redirect(
                    'originadmin/origin/index'
                );
            }
        }

        // process $_POST data if the form was submitted
        if ($postData = $this->getRequest()->getPost('originData')) {
            try {
                $origin->addData($postData);
                $origin->save();

                $this->_getSession()->addSuccess(
                    $this->__('The origin has been saved.')
                );

                // redirect to remove $_POST data from the request

                return $this->_redirect(
                    'originadmin/origin/edit',
                    array('id' => $origin->getId())
                );
            } catch (Exception $e) {
                Mage::logException($e);
                $this->_getSession()->addError($e->getMessage());
            }

            /**
             * If we get to here, then something went wrong. Continue to
             * render the page as before, the difference this time being
             * that the submitted $_POST data is available.
             */
        }

        // Make the current brand object available to blocks.
        Mage::register('current_origin', $origin);

        // Instantiate the form container.
        $originEditBlock = $this->getLayout()->createBlock('dos/adminhtml_origin_edit');

        // Add the form container as the only item on this page.
        $this->loadLayout()
            ->_addContent($originEditBlock)
            ->renderLayout();
        
    }

    public function deleteAction()
    {
        $origin = Mage::getModel('dos/origin');

        if ($originId = $this->getRequest()->getParam('id', false)) {
            $origin->load($originId);
        }

        if (!$originId->getId())
        {
            $this->_getSession()->addError(
                $this->__('This origin no longer exists.')
            );

            return $this->_redirect(
                'originadmin/origin/index'
            );
        }

        try {
            $origin->delete();

            $this->_getSession()->addSuccess(
                $this->__('The origin has been deleted.')
            );
        } catch (Exception $e) {
            Mage::logException($e);
            $this->_getSession()->addError($e->getMessage());
        }

        return $this->_redirect(
            'originadmin/origin/index'
        );
    }

    /**
     * Thanks to Ben for pointing out this method was missing. Without
     * this method the ACL rules configured in adminhtml.xml are ignored.
     */
    protected function _isAllowed()
    {
        /**
         * we include this switch to demonstrate that you can add action
         * level restrictions in your ACL rules. The isAllowed() method will
         * use the ACL rule we have configured in our adminhtml.xml file:
         * - acl
         * - - resources
         * - - - admin
         * - - - - children
         * - - - - - smashingmagazine_branddirectory
         * - - - - - - children
         * - - - - - - - brand
         *
         * eg. you could add more rules inside brand for edit and delete.
         */
        $actionName = $this->getRequest()->getActionName();
        switch ($actionName) {
            case 'index':
            case 'edit':
            case 'delete':
                // intentionally no break
            default:
                $adminSession = Mage::getSingleton('admin/session');
                $isAllowed = $adminSession
                    ->isAllowed('dos/origin');
                break;
        }

        return $isAllowed;
    }
}